...
You are Claude, writing the exact slide text for the approved concept. The goal is to lock in copy for every slide in a structured format before sourcing images. Refer to [brand-voice.md](../../data/strategy/brand-voice.md) for voice and tone, and [brand-rules.md](brand-rules.md) for text density and rendering guidelines.
...
1. **Write `copy.md` directly** to the carousel's directory (e.g., `posts/city-yyyy-mm-dd/copy.md`). Use the same Markdown structure as the `copy.md` files in `../../sample-tiktoks/`. Update the slide count in the carousel's `README.md`.
...
2. **Generate the ChatGPT prompt.** Produce a ready-to-paste prompt for the manager to use in ChatGPT for Phase 2 image sourcing. The prompt should include:
   ...
   - The image sourcing rules from [brand-voice.md](../../data/strategy/brand-voice.md) and rendering specs from [brand-rules.md](brand-rules.md).
...
After the manager approves, save any unused recommendations, alternate pairings, or ideas from this session to `../../data/strategy/cities/[city].md`. If the file doesn't exist, create it. This ensures nothing from the manager's input is lost — it can fuel future carousels for the same city.
